...
const deleteCompany = (event: Event, id: CompanyId) => {
  ...
  <div class="w-full h-full flex flex-col gap-6">
    ...
    <div class="flex items-center justify-between gap-4">
      <div class="flex flex-col items-start">
        <h1 class="text-2xl font-bold">銘柄ギャラリー</h1>
        <p class="text-sm text-gray-400">
          登録済みの銘柄から選択、または新しく銘柄を追加します
        </p>
      </div>
      ...
      <div class="flex items-center gap-3 shrink-0">
        ...
        <button
          type="button"
          class="flex items-center gap-1 text-xs text-gray-400 hover:text-[#d5c5a9] transition-colors shrink-0"
          @click="emit('showArchive')"
        >
    ...
    >
      ...
      <button
        type="button"
        class="group flex flex-col items-center justify-center gap-2 h-40 rounded-lg border-2 border-dashed border-gray-700 text-gray-400 hover:border-[#d5c5a9] hover:text-[#d5c5a9] transition-colors"
        @click="emit('upload')"
      >
        <Plus class="w-6 h-6" />
        <span class="text-xs font-bold">銘柄を追加</span>
      </button>
      ...
      <div
        ...
        :key="company.id"
        class="group relative flex flex-col justify-between h-40 rounded-lg bg-[#1f2020] p-4 text-start cursor-pointer border border-transparent hover:border-[#d5c5a9]/60 hover:bg-[#242626] transition-colors"
        @click="selectCompany(company.id)"
      >
        <Trash2
          class="w-4 h-4 absolute top-3 right-3 text-gray-500 opacity-0 group-hover:opacity-100 hover:text-red-400 transition-opacity"
          @click="(e) => deleteCompany(e, company.id)"
        />
    ...
    >
      ...
      <p class="text-sm">まだ銘柄が登録されていません</p>
      <p class="text-xs">「銘柄を追加」から企業を検索して登録しましょう</p>
    </div>
  </div>
</template>

...
- 最大5件
- 対象とするのは以下の2種類のみ:
  A) 本文で明示的に言及されている懸念・下方修正要因
  B) 開示された数値・実績から、グロース株投資家として
     合理的に推測できる「事業上の」懸念事項
- 【重要】以下は risks に含めない（絶対に書かない）:
  - 「持続性を判断する材料が不足している」等、分析者側の
    判断材料の欠如を理由にした懸念
  - ガイダンスが「示されていない」こと自体を懸念とすること
    （ガイダンスの中身が弱い場合は対象にしてよいが、
    「ガイダンスがない」という事実だけを理由にしない）
- risksは常に「実績の数値が示す事業上のトレンド・構造」を根拠にする
  - 悪い例:「セグメント別開示がなく成長の中身が分からない」
    （→ これは開示不足の指摘であり不可）
  - 良い例:「粗利率が前年から2pt低下しており、
    高性能製品への投資負担が利益率を圧迫し始めている可能性」
    （→ 開示された数値そのものから読み取れる懸念）
- 具体的に書く
- 重複禁止
- なければ空配列 []
